...
func TestReporter1JSON(t *testing.T) {
	...
	// reporter
	reportHandler1 := NewReportHandler1(report1.NewService(db))
	...
	reportHandler1.JSON(w, req)

	assert.Equal(t, http.StatusOK, w.Code)
	assert.Equal(t, testJSON1, strings.TrimSpace(w.Body.String()))
}
